...
SRC_VOCAB=$MONO_PATH/vocab.de.$CODES
TGT_VOCAB=$MONO_PATH/vocab.hsb.$CODES
...
TGT_TEST=$PARA_PATH/devel_test.hsb-de.hsb
...
# if ! [[ "$(wc -l < $SRC_RAW)" -eq "$N_MONO" ]]; then echo "ERROR: Number of lines doesn't match! Be sure you have $N_MONO sentences in your DE monolingual data."; exit; fi
# if ! [[ "$(wc -l < $TGT_RAW)" -eq "$N_MONO" ]]; then echo "ERROR: Number of lines doesn't match! Be sure you have $N_MONO sentences in your HSB monolingual data."; exit; fi
...
echo "BPE codes applied to DE in: $SRC_TOK.$CODES"
echo "BPE codes applied to HSB in: $TGT_TOK.$CODES"
...
# cat $SRC_VALID | $NORM_PUNC -l de | $REM_NON_PRINT_CHAR | $TOKENIZER -l de -no-escape -threads $N_THREADS > $SRC_VALID
# cat $TGT_VALID |  $NORM_PUNC -l de | $REM_NON_PRINT_CHAR | $TOKENIZER -l en -no-escape -threads $N_THREADS > $TGT_VALID
# cat $SRC_TEST |  $NORM_PUNC -l de | $REM_NON_PRINT_CHAR | $TOKENIZER -l de -no-escape -threads $N_THREADS > $SRC_TEST
# cat $TGT_TEST |  $NORM_PUNC -l de | $REM_NON_PRINT_CHAR | $TOKENIZER -l en -no-escape -threads $N_THREADS > $TGT_TEST
...
echo "    DE: $SRC_TOK.$CODES.pth"
echo "    HSB: $TGT_TOK.$CODES.pth"
echo "Parallel validation data:"
echo "    DE: $SRC_VALID.$CODES.pth"
echo "    HSB: $TGT_VALID.$CODES.pth"
echo "Parallel test data:"
echo "    DE: $SRC_TEST.$CODES.pth"
echo "    HSB: $TGT_TEST.$CODES.pth"
echo ""
